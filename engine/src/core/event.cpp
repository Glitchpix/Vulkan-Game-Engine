...
bool EventManager::unregister_event(EventCode code, void* listener, event_callback callback) {
    try {
        ...
        MSG_WARN("Listener: %p tried to unregister non-registered event callback: %p with code: %x, Error: %s", static_cast<void*>(listener), reinterpret_cast<void*>(callback), static_cast<int>(code), e.what());
        return false;
    }
