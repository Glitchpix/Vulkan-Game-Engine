...
private:
    ...
    void drawFrame() {
        ...
        uint32_t imageIndex;
        VkResult resultImageAcquire = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);

        if (resultImageAcquire == VK_ERROR_OUT_OF_DATE_KHR) {
            recreateSwapChain();
            return;
        } else if (resultImageAcquire != VK_SUCCESS && resultImageAcquire != VK_SUBOPTIMAL_KHR) {
        ...
        resultImageAcquire = vkQueuePresentKHR(presentQueue, &presentInfo);

        if (resultImageAcquire == VK_ERROR_OUT_OF_DATE_KHR || resultImageAcquire == VK_SUBOPTIMAL_KHR || framebufferResized) {
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        // ubo.proj = glm::ortho(-aspect, aspect, -1.0f, 1.0f, -5.0f, 5.0f);
    ...
    void createUniformBuffers() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(bufferSize, usageFlags, propertyFlags, uniformBuffers[i], uniformBuffersMemory[i]);
    ...
    VkFormat findDepthFormat() {
        ...
        return findSupportedFormat(candidates, tiling, featureFlags);
    }
    ...
    void createDepthResources() {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, tiling, usageFlags, memoryFlags, depthImage, depthImageMemory);
    ...
    void createTextureImage() {
        ...
        stbi_uc* pixels = stbi_load("textures/aiForest.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
        ...
        VkBuffer stagingBuffer;
        VkDeviceMemory stagingBufferMemory;
        createVkStageBuffer(imageSize, stagingBuffer, stagingBufferMemory, pixels);
        ...
        createImage(texWidth, texHeight ,format, tiling, usageFlags, imageMemoryProperties, textureImage, textureImageMemory);

        VkImageLayout initalLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        VkImageLayout dstTransferLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
        VkImageLayout optimalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        transitionImageLayout(textureImage, format, initalLayout, dstTransferLayout);
        copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        transitionImageLayout(textureImage, format, dstTransferLayout, optimalLayout);
    ...
    void createImage(uint32_t texWidth, uint32_t texHeight, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usageFlags, 
                     VkMemoryPropertyFlags memoryProperties, VkImage& image, VkDeviceMemory& textureImageMemory) {
